...
public:
    ...
    range_class begin() const
    {
        return 0;
    }
    range_class end() const
    {
        return m_end;
    }
    void operator++()
    ...
    bool operator!=(const range_class &e) const
    {
        return  m_end < e.m_end;
    }
private:
    Int m_end;
};
...
void build_norm()
{
    ...
    for(auto i : range(sz-1))
    {
        ...
        {
            addnorm(napkin[i+0][j+0],
                    napkin[i+0][j+1],
                    napkin[i+1][j+0]);
            addnorm(napkin[i+0][j+1],
                    napkin[i+1][j+1],
                    napkin[i+0][j+0]);
            addnorm(napkin[i+1][j+1],
                    napkin[i+1][j+0],
                    napkin[i+0][j+1]);
            addnorm(napkin[i+1][j+0],
                    napkin[i+0][j+0],
                    napkin[i+1][j+1]);
        }
    }
...
//       std::cout<<"draw"<< napkin[sz-1][sz-1].position.p[0] << " " << napkin[sz-1][sz-1].position.p[1] << " " << napkin[sz-1][sz-1].position.p[2] <<std::endl;
...

{
    ...
    auto doit=[&allforcestb,&num](size_t m,size_t r){
        ...
        for(auto i : range(sz-1)) if (i%m==r)
        {
            for(auto j : range(sz-1))
            {
...
//        auto f=std::async( std::launch::async ,doit,tasks,task);
    ...
    std::cout << "nb_threads " << allforces.size() << std::endl;
        ...
        //  std::cout<<"tick"<< napkin[sz-1][sz-1].position.p[0] << " " << napkin[sz-1][sz-1].position.p[1] << " " << napkin[sz-1][sz-1].position.p[2] <<std::endl;
        const auto t1 = glutGet(GLUT_ELAPSED_TIME);
        compute_time += t1-t;
